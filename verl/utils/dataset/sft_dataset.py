...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # string
        # print("DEBUG prompt_chat type:", type(prompt_chat))
        # if isinstance(prompt_chat, list):
        #     print("  first element type:", type(prompt_chat[0]))
        #     if isinstance(prompt_chat[0], dict):
        #         print("  keys in first element:", list(prompt_chat[0].keys()))
        #         print("  first content type:", type(prompt_chat[0].get('content')))
        # else:
        #     print("  prompt_chat value:", prompt_chat)
        # --- Fix: convert numpy content to str ---
        for msg in prompt_chat:
            if isinstance(msg, dict):
                c = msg.get("content", None)
                if isinstance(c, np.ndarray):
                    # 把 numpy.ndarray 转成 string 表达
                    try:
                        msg["content"] = str(c.tolist())
                    except Exception:
                        msg["content"] = str(c)
