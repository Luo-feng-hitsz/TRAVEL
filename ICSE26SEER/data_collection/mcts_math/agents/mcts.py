...
class MCTS(SBSREACT):
    ...
    def create_child(
        ...
        if parser_result is None:
            ...
            # observation = parser_result["action_input"]

            # if self.config.verbose:
            #     print(colored(f"{observation}\n", OBSERVATION_COLOR))
            step_result = step_result.strip()
            if '<step>' in step_result:
                step_result = step_result.split('<step>')[0]
            new_node.state["text"] = f"{step_result}\n</step>\n<step>" #{observation}"
    ...
    def eval_final_answer(self, node: Type[MCTSNode]) -> None:
        ...
        if self.ground_truth:
            ...
            correct = is_equiv(self.test_cases, final_answer)
            # backup
